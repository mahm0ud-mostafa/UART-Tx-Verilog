module serializer #(parameter W = 8)(                   // W: data width parameter, default value is 8
    input wire CLK, RST, LOAD, SHIFT_EN,                // input signals (1-bit each)
    input wire [W-1:0] P_DATA,                          // parallel data input (W-bit bus)
    output wire S_DATA);                                // serial data output (1-bit wire)
    reg [W-1:0] SHIFT_REG;
    always@(posedge CLK or negedge RST) begin           // falling edge asynch. reset triggers the always block
