module main_controller #(parameter W = 8)(    
    input wire CLK, RST, V_INPUT, P_EN,              
    output reg LOAD, SHIFT_EN, BUSY,
    output reg [1:0] MUX_SEL);                          // MUX_SEL controls which bit is sent on Tx o/p
    localparam [2:0] IDLE = 3'b000, START= 3'b001, DATA = 3'b010, PARITY = 3'b011, STOP = 3'b100;   // states binary encoding
    reg [2:0] current_state, next_state;
    localparam COUNT_W = (W<=1)?1:$clog2(W);     // required width for data bit counter (searched for clog)
    reg [COUNT_W-1:0] bit_count;
    reg p_en_reg;                                       // saves P_EN for the current frame
    always@(posedge CLK or negedge RST) begin           // state, counter and parity enable registers
