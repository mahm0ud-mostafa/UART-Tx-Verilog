module mux(
    input wire S_DATA, PARITY_BIT,   
    input wire [1:0] MUX_SEL,            // select signal (coming from the controller)
    output reg TX_OUTPUT);               // final UART serial output (1bit)
    always@(*) begin                     // mux output logic (combinational)
        case(MUX_SEL)
